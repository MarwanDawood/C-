classdef SPIRegisterWrite < matlabshared.svd.SPIMasterBlock ...
        ...
        if ~coder.target('Rtw') && ~coder.target('Sfun')
            if ~isempty(obj.Hw)
                if ~isValidSlaveSelectPin(obj.Hw,obj.SPIModule,value)
                    error(message('svd:svd:PinNotFound',value,'SPI Register Write'));
                end
            end
        end
        obj.Pin = value;
    ...
    methods(Static, Access=protected)
        ...
        header = matlab.system.display.Header(mfilename('class'),...
            ...
            'Text', ['Write data to registers of an SPI slave device.' char(10) char(10) ...
            'Initiate an SPI write sequence starting with the SPI slave register address followed by block input data.' char(10) char(10) ...
            'The block accepts the values as an [Nx1] or [Nx1] array.']);
        end
